...
from tagolym._typing import Any, FilePath, Optional
    ...
    with mlflow.start_run(run_name=run_name):
        ...
        with tempfile.TemporaryDirectory() as dp:
            ...
            utils.save_dict(
                ...
                Path(dp, "train_metrics.json"),
                cls=utils.NumpyEncoder,
            )
            utils.save_dict(
                artifacts["val_metrics"],
                Path(dp, "val_metrics.json"),
                cls=utils.NumpyEncoder,
            )
            utils.save_dict(
                artifacts["test_metrics"],
                Path(dp, "test_metrics.json"),
                cls=utils.NumpyEncoder,
            )
            utils.save_dict(
                {**args.__dict__}, Path(dp, "args.json"), cls=utils.NumpyEncoder
            )
...
def optimize(args_fp: FilePath, study_name: str, num_trials: int) -> None:
    ...
    # save to config
    utils.save_dict(
        dict(args), Path(config.CONFIG_DIR, "args_opt.json"), cls=utils.NumpyEncoder
    )
